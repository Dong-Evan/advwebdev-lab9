...
def create_quiz():
    ...
    # Use the service to create a quiz with the `data` and store the returned
    # quiz ID in `quiz_id`
    ...
    # Use jsonify to create a JSON response containing `message` and `quiz_id`,
...
def submit_quiz(quiz_id):
    ...
    # and store in `user_answers`
    user_answers = request.json.get('answers')

    # Call `evaluate_quiz` with `quiz_id` and `user_answers` and store the
    # result in `score` and `message`
    score, message = quizService.evaluate_quiz(quiz_id, user_answers)

    # If `score` is None, return an error with status 404. Otherwise, return
    # `score` and `message` with status 200.
    if score is None:
