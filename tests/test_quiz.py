...
def test_create_quiz(mock_create_quiz, client):
    ...
    # Assert that status code is 201, `quiz_id` in response is 1, and `mock_create_quiz` was called once
...
def test_get_quiz(mock_get_quiz, client):
    # Create a MagicMock named `mock_quiz`, set `title` to "Sample Quiz", and `questions` to a sample list
    mock_quiz = MagicMock(title = 'Sample Quiz', questions = [{'text': 'q1', 'answer': 'a1'}])
    # mock_quiz = {
    #     'title': 'Sample Quiz', 
    #     'questions': [
    #         {'text': 'q1', 'answer': 'a1'}]
    ...
    # Assert that status code is 200, `title` in response is "Sample Quiz", and `mock_get_quiz` was called once
...
def test_submit_quiz(mock_evaluate_quiz, client):
    ...
    # Set `mock_evaluate_quiz.return_value` to (1, "Quiz evaluated successfully")
    mock_evaluate_quiz.return_value = (1, "Quiz evaluated successfully")
    
    # Use `client.post` to send a POST request to `/api/quizzes/1/submit` with JSON data containing answers
    response = client.post(
        '/api/quizzes/1/submit',
        json = {'answers': ['a1']}
    )

    # Assert that status code is 200, `score` in response is 1, `message` is "Quiz evaluated successfully", and `mock_evaluate_quiz` was called once
    assert response.status_code == 200
